...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, AttentionMixin):
    ...
    # FreeFuse methods
    # ──────────────────────────────────────────────────────────────
    def set_freefuse_masks(self, image_masks: Dict[str, torch.Tensor]):
        """Store image-space LoRA masks. Converted to unified coords in forward().

        Args:
            image_masks: ``{lora_name: (B, img_token_count)}``  0/1 float masks.
        """
        self._freefuse_image_masks = image_masks

    def set_freefuse_token_pos_maps(
        self, token_pos_maps: Dict[str, List[List[int]]]
    ):
        """Store text-relative token position maps.

        Args:
            token_pos_maps: ``{lora_name: [[positions_for_prompt_0], ...]}``
                            Positions are 0-indexed into the text (cap) portion.
        """
        self._freefuse_token_pos_maps = token_pos_maps

    def reset_freefuse(self):
        """Clear all FreeFuse state."""
        for attr in (
            "_freefuse_image_masks",
        ...
        for _, module in self.named_modules():
            if isinstance(module, FreeFuseLinear):
                module.freefuse_masks = None
                module.freefuse_token_pos_maps = None

    def _apply_unified_masks(
        self,
        x_seqlens: List[int],
        cap_seqlens: List[int],
        max_unified_seqlen: int,
    ):
        """Convert image masks → unified-sequence masks and set on LoRA layers."""
        if (
            not hasattr(self, "_freefuse_image_masks")
            or self._freefuse_image_masks is None
        ):
            return
        batch_size = len(x_seqlens)
        device = next(iter(self._freefuse_image_masks.values())).device

        unified_masks: Dict[str, torch.Tensor] = {}
        for lora_name, img_mask in self._freefuse_image_masks.items():
            if img_mask.dim() == 3:
                img_mask = img_mask.squeeze(-1)  # (B, img_tok)
            mask_B = img_mask.shape[0]
            parts = []
            for b in range(batch_size):
                mb = b % mask_B  # cycle if CFG doubles batch
                x_len = x_seqlens[b]
                cap_len = cap_seqlens[b]
                total = x_len + cap_len
                row = torch.zeros(max_unified_seqlen, device=device)
                # Image part (pad SEQ_MULTI_OF tail with 0)
                valid = min(img_mask.shape[1], x_len)
                row[:valid] = img_mask[mb, :valid]
                # Text part: all 1s (allow all LoRAs on text tokens)
                row[x_len : x_len + cap_len] = 1.0
                parts.append(row)
            unified_masks[lora_name] = torch.stack(parts).unsqueeze(-1)  # (B, S, 1)

        # Only apply masks to sequence-processing layers inside the main
        # transformer layers (self.layers.*).  This excludes:
        #   - adaLN_modulation / t_embedder / cap_embedder (2D timestep tensors)
        #   - noise_refiner / context_refiner / siglip_refiner (separate seqs)
        #   - all_final_layer and other non-unified-sequence modules
        _SEQ_KEYWORDS = ('attention', 'feed_forward')
        for name, module in self.named_modules():
            if isinstance(module, FreeFuseLinear):
                in_main = 'layers.' in name
                in_seq  = any(kw in name for kw in _SEQ_KEYWORDS)
                if in_main and in_seq:
                    module.set_freefuse_masks(unified_masks)
                else:
                    module.set_freefuse_masks(None)

    def _apply_unified_token_pos_maps(self, x_seqlens: List[int]):
        """Convert text-relative positions → unified-sequence positions on LoRA layers."""
        if (
            not hasattr(self, "_freefuse_token_pos_maps")
            or self._freefuse_token_pos_maps is None
        ):
            return
        batch_size = len(x_seqlens)
        unified_pos: Dict[str, List[List[int]]] = {}
        for lora_name, positions_list in self._freefuse_token_pos_maps.items():
            unified_pos[lora_name] = []
            for b in range(batch_size):
                src = positions_list[min(b, len(positions_list) - 1)]
                unified_pos[lora_name].append(
                    [p + x_seqlens[b] for p in src]
                )
        _SEQ_KEYWORDS = ('attention', 'feed_forward')
        for name, module in self.named_modules():
            if isinstance(module, FreeFuseLinear):
                ...
                in_seq  = any(kw in name for kw in _SEQ_KEYWORDS)
                if in_main and in_seq:
                    module.set_freefuse_token_pos_maps(unified_pos)
                else:
                    module.set_freefuse_token_pos_maps(None)

    def _setup_freefuse_processors(
        self,
        x_seqlens: List[int],
        cap_seqlens: List[int],
        joint_attention_kwargs: Optional[Dict] = None,
    ):
        """Push sequence-boundary info & optional bias onto every FreeFuse processor."""
        jk = joint_attention_kwargs or {}
        token_pos_maps = jk.get("freefuse_token_pos_maps", None)
        top_k_ratio = jk.get("top_k_ratio", 0.1)
        eos_token_index = jk.get("eos_token_index", None)
        bg_positions = jk.get("background_token_positions", None)
        attn_bias = jk.get("attention_bias", None)
        attn_bias_blocks = jk.get("attention_bias_blocks", None)

        for layer_idx, layer in enumerate(self.layers):
            proc = layer.attention.processor
            if not isinstance(proc, FreeFuseZImageAttnProcessor):
                continue
            proc._x_seqlens = x_seqlens
            proc._cap_seqlens = cap_seqlens
            proc._top_k_ratio = top_k_ratio
            proc._eos_token_index = eos_token_index
            proc._background_token_positions = bg_positions
            proc._freefuse_token_pos_maps = token_pos_maps
            # Per-block attention bias
            if attn_bias is not None:
                block_name = f"layers.{layer_idx}"
                if attn_bias_blocks is None or block_name in attn_bias_blocks:
                    proc._attention_bias = attn_bias
                else:
                    proc._attention_bias = None
            else:
                proc._attention_bias = None

    def _collect_concept_sim_maps(self) -> Optional[Dict[str, torch.Tensor]]:
        """Retrieve concept_sim_maps from whichever processor computed them."""
        for layer in self.layers:
            proc = layer.attention.processor
            if (
                isinstance(proc, FreeFuseZImageAttnProcessor)
                and proc.concept_sim_maps is not None
            ):
                maps = proc.concept_sim_maps
                proc.concept_sim_maps = None
                return maps
        return None
    ...
    def forward(
        ...
        # ── FreeFuse: push info to processors & LoRA layers ──
        if not omni_mode:
            self._setup_freefuse_processors(
                x_seqlens, cap_seqlens, joint_attention_kwargs
            )
            self._apply_unified_masks(
                x_seqlens, cap_seqlens, unified.shape[1]
            )
            self._apply_unified_token_pos_maps(x_seqlens)
        ...
        # ── FreeFuse: collect concept sim maps ──
        concept_sim_maps = self._collect_concept_sim_maps() if not omni_mode else None
