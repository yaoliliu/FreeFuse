...
class Flux2Transformer2DModel(
    ...
    def set_freefuse_token_pos_maps(
        ...
        """
        Set concept token position maps for all FreeFuse attention processors.
        ...
        for name, module in self.named_modules():
            if hasattr(module, 'processor'):
                ...
                if hasattr(processor, '_freefuse_token_pos_maps'):
                    processor._freefuse_token_pos_maps = freefuse_token_pos_maps
    ...
    def set_freefuse_masks(
        ...
        """
        Set per-concept spatial masks for FreeFuseLinear layers.
    ...
    def set_freefuse_attention_bias(
        ...
        """
        Set additive attention bias for cross-LoRA suppression.
        
        Args:
            attention_bias: (B, seq, seq) additive bias tensor
        """
        for name, module in self.named_modules():
            if hasattr(module, 'processor'):
                processor = module.processor
                if hasattr(processor, '_attention_bias'):
                    processor._attention_bias = attention_bias

    def set_freefuse_txt_len(self, txt_len: int) -> None:
        """
        Set text sequence length for single-stream processors.
    ...
    def set_freefuse_background_info(
        ...
        """
        Set background detection info for all FreeFuse processors.
        
        Args:
            eos_token_index: Index of first EOS token
        ...
        for name, module in self.named_modules():
            if hasattr(module, 'processor'):
                ...
                if hasattr(processor, '_eos_token_index'):
                    processor._eos_token_index = eos_token_index
                if hasattr(processor, '_background_token_positions'):
                    processor._background_token_positions = background_token_positions
    ...
    def get_concept_sim_maps(self) -> Optional[Dict[str, torch.Tensor]]:
        """
        Retrieve concept similarity maps from the designated extraction block.
        
        Returns:
            Dict mapping lora_name -> similarity map tensor, or None
        """
        for name, module in self.named_modules():
            if hasattr(module, 'processor'):
                processor = module.processor
                if hasattr(processor, 'concept_sim_maps') and processor.concept_sim_maps is not None:
                    return processor.concept_sim_maps
        return None

    def enable_concept_sim_map_extraction(self, block_name: str = None) -> None:
        """
        Enable concept similarity map extraction on a specific block.
        
        Args:
            block_name: Name of the block (e.g., "transformer_blocks.4" or "single_transformer_blocks.10")
                       If None, enables on the last double-stream block.
        """
        # First disable all
        for name, module in self.named_modules():
            if hasattr(module, 'processor'):
        ...
        # Enable on target block
        if block_name is None:
            # Default to last double-stream block
            block_name = f"transformer_blocks.{len(self.transformer_blocks) - 1}"
        
        for name, module in self.named_modules():
            if block_name in name and hasattr(module, 'processor'):
                processor = module.processor
                if hasattr(processor, 'cal_concept_sim_map'):
