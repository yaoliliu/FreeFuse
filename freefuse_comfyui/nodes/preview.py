...
Background mask (if present) is always gray."""
    ...
    def preview(self, masks, latent):
        # Get dimensions from latent
        samples = latent.get("samples", None)
        if samples is None or len(samples.shape) < 2:
            raise ValueError("Invalid latent: missing 'samples' tensor")
        
        # Latent is (B, C, H, W), need to upscale to pixel dimensions
        latent_h = samples.shape[2]
        latent_w = samples.shape[3]
        # VAE typically downsamples 8x
        height = latent_h * 8
        width = latent_w * 8
        
        # Handle different mask data structures
        # FREEFUSE_MASKS can be: {"masks": {...}, "similarity_maps": {...}}
        # Or from grid extractor: might have nested dicts
        mask_dict = masks.get("masks", {})

        # If mask_dict is empty, try the masks data directly
        if not mask_dict:
            mask_dict = masks
        ...
        for name in mask_dict.keys():
            # Skip if value is not a tensor (could be nested dict)
            value = mask_dict[name]
    ...
    def _create_combined_preview(
        ...
        # Find first valid tensor to get device
        device = None
        for name in sorted_names:
            value = mask_dict.get(name)
            if hasattr(value, 'device'):
                device = value.device
                break
        
        if device is None:
            # No valid tensors found, return empty
