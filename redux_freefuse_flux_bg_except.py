"""
Main script using FreeFuseFluxAttnProcessor with Redux for global style control.

This script combines:
1. Redux: Global style/reference image conditioning
2. FreeFuse with LoRAs: Multi-concept generation with attention-based concept separation

The Redux embeddings are concatenated with text prompt embeddings to provide
global visual guidance, while LoRAs handle individual concept appearance.
"""
import os
import torch
...
def load_reference_image(image_path):
    """Load a reference image for Redux conditioning.
...
def setup_redux_pipeline():
    """Initialize the Redux prior pipeline for image conditioning.
    
    Returns:
        FluxPriorReduxPipeline ready for use
    """
...
def get_redux_embeddings(
    ...
    """Get Redux embeddings from a reference image.
    
    Args:
        pipe_prior_redux: FluxPriorReduxPipeline instance
        reference_image: PIL Image or path to image
        style_prompt: Optional text prompt to associate with the image (e.g., "van gogh style")
        prompt_embeds_scale: Scaling factor for prompt embeddings (default 1.0)
        pooled_prompt_embeds_scale: Scaling factor for pooled embeddings (default 1.0)
    ...
    if isinstance(reference_image, str):
        reference_image = load_reference_image(reference_image)
    
    # Get Redux embeddings
    redux_output = pipe_prior_redux(
        reference_image,
        prompt=style_prompt,
...
def combine_text_and_redux_embeds(
    text_prompt_embeds,
    text_pooled_embeds,
    redux_prompt_embeds,
    redux_pooled_embeds,
    redux_pooled_weight=1.0,
):
    """Combine text prompt embeddings with Redux image embeddings.
    
    The text embeddings are concatenated with Redux embeddings along the sequence dimension.
    The pooled embeddings are added together (with optional weighting for Redux).
    
    Args:
        text_prompt_embeds: Text prompt embeddings from T5 encoder, shape (B, text_seq_len, D)
        text_pooled_embeds: Pooled text embeddings from CLIP, shape (B, D)
        redux_prompt_embeds: Redux image embeddings, shape (B, redux_seq_len, D)
        redux_pooled_embeds: Redux pooled embeddings, shape (B, D)
        redux_pooled_weight: Weight for Redux pooled embeddings when adding (default 1.0)
        
    Returns:
        combined_prompt_embeds: (B, text_seq_len + redux_seq_len, D)
        combined_pooled_embeds: (B, D)
    """
    # Concatenate prompt embeddings: text first, then redux
    # This preserves text token positions for find_concept_positions
    combined_prompt_embeds = torch.cat([
        text_prompt_embeds,
        redux_prompt_embeds,
    ], dim=1)
    
    # Add pooled embeddings with optional weighting
    combined_pooled_embeds = text_pooled_embeds + redux_pooled_weight * redux_pooled_embeds
...
def main():
    # ========== Configuration ==========
    # Redux configuration - global style conditioning
    use_redux = True  # Set to False to disable Redux
    reference_image_path = "optional_condition/style.png"  # Path to reference image for style
    # style_prompt = "Traditional Japanese ukiyo-e woodblock style in the style of Hokusai, a dramatic great wave with curling foam crests towering over small boats. Deep Prussian blue and indigo waves with white spray forming claw-like patterns. Mount Fuji visible in the distant background beneath an overcast sky"  # Optional: text description of the style
    style_prompt = "ukiyo-e woodblock style in the style of Hokusai"  # Optional: text description of the style
    redux_prompt_embeds_scale = 0.25  # Scale for Redux prompt embeddings
    redux_pooled_embeds_scale = 0.3  # Scale for Redux pooled embeddings
    redux_pooled_weight = 0.0  # Weight when adding Redux pooled to text pooled
    
    # LoRA configuration
    lora_configs = [
        {"path": "loras/shalnark_flux.safetensors", "name": "conan", "weight": 0.7},
        {"path": "loras/spongebobs.safetensors", "name": "pikachu", "weight": 0.7},
    ]
    ...
    t5_prompt = "A picture of two character: shalnark, an anime boy with blonde bob haircut and turquoise eyes, wearing purple and teal futuristic uniform, determined expression, digital anime art style and spongebob, a cheerful, optimistic yellow sea sponge, square-shaped body covered in porous holes, large expressive blue eyes with three eyelashes each, prominent two front teeth, wearing a white collared shirt, red tie, and brown square pants, the background is Traditional Japanese ukiyo-e woodblock style in the style of Hokusai"
    negative_prompt = ""
    ...
    background_concept = "Traditional Japanese ukiyo-e woodblock style in the style of Hokusai"
    ...
    # Setup Redux pipeline if enabled
    pipe_prior_redux = None
    if use_redux:
        print("Loading Redux prior pipeline...")
        pipe_prior_redux = setup_redux_pipeline()
    ...
    # Get Redux embeddings and combine if enabled
    if use_redux and pipe_prior_redux is not None:
        print("Getting Redux embeddings from reference image...")
        redux_embeds = get_redux_embeddings(
            pipe_prior_redux,
            reference_image_path,
            style_prompt=style_prompt,
            prompt_embeds_scale=redux_prompt_embeds_scale,
            pooled_prompt_embeds_scale=redux_pooled_embeds_scale,
        )
        print(f"  Redux prompt embeds shape: {redux_embeds['prompt_embeds'].shape}")
        
        # Combine text and Redux embeddings
        prompt_embeds, pooled_prompt_embeds = combine_text_and_redux_embeds(
            prompt_embeds,
            pooled_prompt_embeds,
            redux_embeds['prompt_embeds'],
            redux_embeds['pooled_prompt_embeds'],
            redux_pooled_weight=redux_pooled_weight,
        )
        print(f"  Combined prompt embeds shape: {prompt_embeds.shape}")

    # ========== Find Concept Positions ==========
    # Note: Concept positions are based on the TEXT prompt tokens only
    # Redux embeddings are concatenated AFTER text embeddings, so positions remain valid
